...
def _sgdc_learning_rate(name):
    return hp.pchoice(
        name, [(0.50, "optimal"), (0.25, "invscaling"), (0.25, "constant")]
    )


def _sgdr_learning_rate(name):
    return hp.pchoice(
        name, [(0.50, "invscaling"), (0.25, "optimal"), (0.25, "constant")]
    )
...
def _svm_hp_space(
    ...
    if kernel in ["linear", "rbf", "sigmoid"]:
        ...
        degree_ = _svm_degree(name_func("degree")) if degree is None else degree
    if kernel in ["linear"]:
        gamma_ = "auto"
    else:
        gamma_ = (
            _svm_gamma(name_func("gamma"), n_features=1) if gamma is None else gamma
        )
    ...
    if kernel in ["linear", "rbf"]:
        ...
        if kernel == "poly":
            coef0_ = hp.pchoice(
                ...
                [
                    (0.3, 0),
                    (0.7, gamma_ * hp.uniform(name_func("coef0val"), 0.0, 10.0)),
                ],
            )
        elif kernel == "sigmoid":
            coef0_ = hp.pchoice(
                ...
                [
                    (0.3, 0),
                    (0.7, gamma_ * hp.uniform(name_func("coef0val"), -10.0, 10.0)),
                ],
            )
        else:
            pass
    ...
    hp_space = dict(
        ...
        coef0=coef0_,
        degree=degree_,
        shrinking=(hp_bool(name_func("shrinking")) if shrinking is None else shrinking),
        tol=_svm_tol(name_func("tol")) if tol is None else tol,
        max_iter=(
            _svm_max_iter(name_func("maxiter")) if max_iter is None else max_iter
        ),
        verbose=verbose,
...
def _svr_hp_space(name_func, epsilon=None):
    ...
    hp_space = {}
    hp_space["epsilon"] = (
        _svm_epsilon(name_func("epsilon")) if epsilon is None else epsilon
    )
    return hp_space
...
#########################################
def svc_kernel(
    name, kernel, class_weight=None, random_state=None, probability=False, **kwargs
):
    """
    ...
    hp_space["class_weight"] = (
        _class_weight(_name("class_weight")) if class_weight is None else class_weight
    )
...
def liblinear_svc(
    ...
    rval = scope.sklearn_LinearSVC(
        ...
        multi_class=(
            hp.choice(_name("multiclass"), ["ovr", "crammer_singer"])
            if multi_class is None
            else multi_class
        ),
        fit_intercept=fit_intercept,
        intercept_scaling=(
            _svm_int_scaling(_name("intscaling"))
            if intercept_scaling is None
            else intercept_scaling
        ),
        class_weight=(
            _class_weight(_name("clsweight"))
            if class_weight == "choose"
            else class_weight
        ),
...
def _knn_hp_space(
    ...
    hp_space = dict(
        n_neighbors=(
            _knn_neighbors(name_func("neighbors"))
            if n_neighbors is None
            else n_neighbors
        ),
        weights=(_knn_weights(name_func("weights")) if weights is None else weights),
        algorithm=algorithm,
        leaf_size=leaf_size,
...
def _trees_hp_space(
    ...
    hp_space = dict(
        ...
        max_depth=(
            _trees_max_depth(name_func("max_depth")) if max_depth is None else max_depth
        ),
        min_samples_split=(
        ...
        bootstrap=(
            _trees_bootstrap(name_func("bootstrap")) if bootstrap is None else bootstrap
        ),
        oob_score=oob_score,
        n_jobs=n_jobs,
        random_state=_random_state(name_func("rstate"), random_state),
...
def random_forest(name, criterion=None, class_weight=None, **kwargs):
    ...
    hp_space["criterion"] = (
        _trees_criterion(_name("criterion")) if criterion is None else criterion
    )
    hp_space["class_weight"] = (
        _trees_class_weight(_name("class_weight"))
...
def _ada_boost_hp_space(
    ...
    hp_space = dict(
        ...
        learning_rate=(
            _ada_boost_learning_rate(name_func("learning_rate"))
...
def ada_boost(name, algorithm=None, **kwargs):
    ...
    hp_space["algorithm"] = (
        _ada_boost_algo(_name("algo")) if algorithm is None else algorithm
    )
...
def ada_boost_regression(name, loss=None, **kwargs):
    ...
    hp_space = _ada_boost_hp_space(_name, **kwargs)
    hp_space["loss"] = _ada_boost_loss(_name("loss")) if loss is None else loss
...
def _grad_boosting_hp_space(
    ...
    hp_space = dict(
        learning_rate=(
            _grad_boosting_learning_rate(name_func("learning_rate"))
            if learning_rate is None
            else learning_rate
        ),
        n_estimators=(
        ...
        subsample=(
            _grad_boosting_subsample(name_func("subsample"))
...
def gradient_boosting(name, loss=None, **kwargs):
    ...
    hp_space["loss"] = _grad_boosting_clf_loss(_name("loss")) if loss is None else loss
...
def decision_tree(
    ...
    rval = scope.sklearn_DecisionTreeClassifier(
        ...
        splitter=hp.choice(_name("splitter"), ["best", "random"])
        if splitter is None
        else splitter,
        ...
        max_depth=max_depth,
        min_samples_split=scope.int(hp.quniform(_name("min_samples_split"), 2, 10, 1))
        if min_samples_split is None
        else min_samples_split,
        min_samples_leaf=scope.int(hp.quniform(_name("min_samples_leaf"), 1, 5, 1))
        if min_samples_leaf is None
        else min_samples_leaf,
        presort=presort,
        random_state=_random_state(_name("rstate"), random_state),
...
def lasso(
    ...
    rval = scope.sklearn_Lasso(
        ...
        max_iter=(_svm_max_iter(_name("maxiter")) if max_iter is None else max_iter),
        tol=_svm_tol(_name("tol")) if tol is None else tol,
        positive=positive,
        selection=hp.choice(_name("selection"), ["cyclic", "random",])
...
def elasticnet(
    ...
    rval = scope.sklearn_ElasticNet(
        alpha=_sgd_alpha(_name("alpha")) if alpha is None else alpha,
        l1_ratio=(_sgd_l1_ratio(_name("l1ratio")) if l1_ratio is None else l1_ratio),
        fit_intercept=fit_intercept,
        normalize=normalize,
        precompute=precompute,
        max_iter=(_svm_max_iter(_name("maxiter")) if max_iter is None else max_iter),
...
def sgd(
    ...
    rval = scope.sklearn_SGDClassifier(
        ...
        max_iter=(_svm_max_iter(_name("maxiter")) if max_iter is None else max_iter),
        learning_rate=(
            _sgdc_learning_rate(_name("learning_rate"))
            if learning_rate is None
            else learning_rate
        ),
...
def sgd_regression(
    ...
    rval = scope.sklearn_SGDRegressor(
        ...
        learning_rate=(
            _sgdr_learning_rate(_name("learning_rate"))
            if learning_rate is None
            else learning_rate
        ),
        eta0=_sgd_eta0(_name("eta0")) if eta0 is None else eta0,
...
def _xgboost_hp_space(
    ...
    hp_space = dict(
        max_depth=(
            _xgboost_max_depth(name_func("max_depth"))
            if max_depth is None
            else max_depth
        ),
        learning_rate=(
            _xgboost_learning_rate(name_func("learning_rate"))
            if learning_rate is None
            else learning_rate
        ),
        n_estimators=(
            _xgboost_n_estimators(name_func("n_estimators"))
            if n_estimators is None
            else n_estimators
        ),
        gamma=(_xgboost_gamma(name_func("gamma")) if gamma is None else gamma),
        min_child_weight=(
            _xgboost_min_child_weight(name_func("min_child_weight"))
            if min_child_weight is None
            else min_child_weight
        ),
        max_delta_step=max_delta_step,
        subsample=(
            _xgboost_subsample(name_func("subsample"))
            if subsample is None
            else subsample
        ),
        colsample_bytree=(
            _xgboost_colsample_bytree(name_func("colsample_bytree"))
            if colsample_bytree is None
            else colsample_bytree
        ),
        colsample_bylevel=(
            _xgboost_colsample_bylevel(name_func("colsample_bylevel"))
            if colsample_bylevel is None
            else colsample_bylevel
        ),
        reg_alpha=(
            _xgboost_reg_alpha(name_func("reg_alpha"))
            if reg_alpha is None
            else reg_alpha
        ),
        reg_lambda=(
            _xgboost_reg_lambda(name_func("reg_lambda"))
            if reg_lambda is None
            else reg_lambda
        ),
        scale_pos_weight=(
            _xgboost_scale_pos_weight(name_func("scale_pos_weight"))
...
def _lightgbm_hp_space(
    ...
    hp_space = dict(
        max_depth=(
            _lightgbm_max_depth(name_func("max_depth"))
            if max_depth is None
            else max_depth
        ),
        num_leaves=(
            _lightgbm_num_leaves(name_func("num_leaves"))
            if num_leaves is None
            else num_leaves
        ),
        learning_rate=(
            _lightgbm_learning_rate(name_func("learning_rate"))
            if learning_rate is None
            else learning_rate
        ),
        n_estimators=(
            _lightgbm_n_estimators(name_func("n_estimators"))
            if n_estimators is None
            else n_estimators
        ),
        min_child_weight=(
            _lightgbm_min_child_weight(name_func("min_child_weight"))
            if min_child_weight is None
            else min_child_weight
        ),
        max_delta_step=max_delta_step,
        subsample=(
            _lightgbm_subsample(name_func("subsample"))
            if subsample is None
            else subsample
        ),
        colsample_bytree=(
            _lightgbm_colsample_bytree(name_func("colsample_bytree"))
            if colsample_bytree is None
            else colsample_bytree
        ),
        reg_alpha=(
            _lightgbm_reg_alpha(name_func("reg_alpha"))
            if reg_alpha is None
            else reg_alpha
        ),
        reg_lambda=(
            _lightgbm_reg_lambda(name_func("reg_lambda"))
            if reg_lambda is None
            else reg_lambda
        ),
        boosting_type=(
            _lightgbm_boosting_type(name_func("boosting_type"))
...
#################################################
def multinomial_nb(
    name, alpha=None, fit_prior=None, class_prior=None,
):
    def _name(msg):
        return "%s.%s_%s" % (name, "multinomial_nb", msg)

    rval = scope.sklearn_MultinomialNB(
        alpha=(hp.quniform(_name("alpha"), 0, 1, 0.001) if alpha is None else alpha),
        fit_prior=(hp_bool(_name("fit_prior")) if fit_prior is None else fit_prior),
...
def passive_aggressive(
    ...
    rval = scope.sklearn_PassiveAggressiveClassifier(
        loss=hp.choice(_name("loss"), ["hinge", "squared_hinge"])
        if loss is None
        else loss,
        C=hp.lognormal(_name("learning_rate"), np.log(0.01), np.log(10),)
        if C is None
        else C,
        fit_intercept=fit_intercept,
        tol=_svm_tol(_name("tol")) if tol is None else tol,
...
def linear_discriminant_analysis(
    ...
    solver_shrinkage = hp.choice(
        _name("solver_shrinkage_dual"),
        [
            ("svd", None),
            ("lsqr", None),
            ("lsqr", "auto"),
    ...
    rval = scope.sklearn_LinearDiscriminantAnalysis(
        ...
        * scope.int(
            hp.qloguniform(
                _name("n_components"), low=np.log(0.51), high=np.log(30.5), q=1.0
            )
        )
...
def quadratic_discriminant_analysis(name, reg_param=None, priors=None):
    def _name(msg):
        return "%s.%s_%s" % (name, "qda", msg)

    rval = scope.sklearn_QuadraticDiscriminantAnalysis(
        reg_param=hp.uniform(_name("reg_param"), 0.0, 1.0)
...
####################################
def one_vs_rest(name, estimator=None, n_jobs=1):
    def _name(msg):
        return "%s.%s_%s" % (name, "one_vs_rest", msg)

    rval = scope.sklearn_OneVsRestClassifier(
        estimator=any_classifier(_name("estimator"))
...
def one_vs_one(name, estimator=None, n_jobs=1):
    def _name(msg):
...
def output_code(name, estimator=None, code_size=1.5, n_jobs=1):
    def _name(msg):
...
def pca(name, n_components=None, whiten=None, copy=True):
    rval = scope.sklearn_PCA(
        ...
        * scope.int(
            hp.qloguniform(
                name + ".n_components", low=np.log(0.51), high=np.log(30.5), q=1.0
            )
        )
        if n_components is None
        else n_components,
...
def standard_scaler(name, with_mean=None, with_std=None):
    rval = scope.sklearn_StandardScaler(
        with_mean=hp_bool(name + ".with_mean",) if with_mean is None else with_mean,
        with_std=hp_bool(name + ".with_std",) if with_std is None else with_std,
    )
    return rval
...
def ts_lagselector(name, lower_lags=1, upper_lags=1):
    rval = scope.ts_LagSelector(
        lag_size=scope.int(
            hp.quniform(name + ".lags", lower_lags - 0.5, upper_lags + 0.5, 1)
        )
    )
    return rval
...
def tfidf(
    ...
    rval = scope.sklearn_Tfidf(
        ...
        lowercase=hp_bool(_name("lowercase"),) if lowercase is None else lowercase,
        max_df=max_df,
        min_df=min_df,
...
def min_max_scaler(name, feature_range=None, copy=True):
    if feature_range is None:
        feature_range = (hp.choice(name + ".feature_min", [-1.0, 0.0]), 1.0)
    rval = scope.sklearn_MinMaxScaler(feature_range=feature_range, copy=copy,)
    return rval


def normalizer(name, norm=None):
    rval = scope.sklearn_Normalizer(
        norm=(hp.choice(name + ".norm", ["l1", "l2"]) if norm is None else norm),
    )
    return rval


def one_hot_encoder(name, n_values=None, categorical_features=None, dtype=None):
    rval = scope.sklearn_OneHotEncoder(
        n_values="auto" if n_values is None else n_values,
        categorical_features=(
            "all" if categorical_features is None else categorical_features
        ),
...
def rbm(
    ...
    rval = scope.sklearn_BernoulliRBM(
        n_components=scope.int(
            hp.qloguniform(
                name + ".n_components", low=np.log(0.51), high=np.log(999.5), q=1.0
            )
        )
        if n_components is None
        else n_components,
        learning_rate=hp.lognormal(name + ".learning_rate", np.log(0.01), np.log(10),)
        if learning_rate is None
        else learning_rate,
        batch_size=scope.int(
            hp.qloguniform(name + ".batch_size", np.log(1), np.log(100), q=1,)
        )
...
def colkmeans(
    ...
    rval = scope.sklearn_ColumnKMeans(
        n_clusters=scope.int(
            hp.qloguniform(
                name + ".n_clusters", low=np.log(1.51), high=np.log(19.5), q=1.0
            )
        )
        if n_clusters is None
        else n_clusters,
        init=hp.choice(name + ".init", ["k-means++", "random"],)
        if init is None
        else init,
        n_init=hp.choice(name + ".n_init", [1, 2, 10, 20],)
        if n_init is None
        else n_init,
        max_iter=scope.int(
            hp.qlognormal(name + ".max_iter", np.log(300), np.log(10), q=1,)
        )
        if max_iter is None
        else max_iter,
        tol=hp.lognormal(name + ".tol", np.log(0.0001), np.log(10),)
        if tol is None
        else tol,
...
def gaussian_random_projection(name, n_components=None, eps=None, random_state=None):
    def _name(msg):
    ...
    rval = scope.sklearn_GaussianRandomProjection(
        n_components=scope.int(
            hp.qloguniform(
                _name("n_components"), low=np.log(0.51), high=np.log(999.5), q=1.0
            )
        )
        if n_components is None
        else n_components,
...
def any_text_preprocessing(name):
    ...
    """
    return hp.choice("%s" % name, [[tfidf(name + ".tfidf")],])
    ...
    model = hp.pchoice(
        ...
        [
            ...
            (
                0.2,
                {
                    "preprocessing": [min_max_scaler(name + ".min_max_scaler")],
